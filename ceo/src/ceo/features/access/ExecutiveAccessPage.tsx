...
async function requestBackend<T>(path: string, token?: string): Promise<T | null> {
 try {
...
const moduleConfigs: Record<ExecutiveModuleKey, ExecutiveModuleConfig> = {
 ...
 actions: ["Review role changes", "Export audit log", "Review module access"],
 },
 "audit-backup": {
 title: "Audit & Backup",
 ...
 actions: ["Review permission matrix", "Review role changes", "Export audit log"],
 },
 products: {
 title: "Products",
...
function toRecordId(prefix: string, value: string | undefined, index: number) {
 return value ? `${prefix}-${value}` : `${prefix}-${index}`;
}

function roleRecords(roles: RoleRecord[]): ExecutiveRecord[] {
 return roles.slice(0, 6).map((role, index) => ({
 id: toRecordId("role", role._id, index),
 title: `${role.name} access policy`,
 owner: role.hasFullAccess ? "System" : "Role Management",
 status: role.hasFullAccess ? "Full Access" : "Scoped",
 value: role.hasFullAccess ? "All permissions" : `${role.permissionKeys.length} permissions`,
 }));
}

function permissionAuditRecords(entries: PermissionAuditRecord[]): ExecutiveRecord[] {
 return entries.slice(0, 6).map((entry, index) => ({
 id: toRecordId("permission-audit", entry._id ?? entry.id, index),
 title: `${entry.action} ${entry.targetType}`,
 owner: entry.actorName ?? "System",
 status: "Logged",
 value: entry.targetName ?? new Date(entry.createdAt).toLocaleDateString(),
 }));
}

function systemAuditRecords(entries: SystemAuditRecord[]): ExecutiveRecord[] {
 return entries.slice(0, 6).map((entry, index) => ({
 id: toRecordId("audit", entry._id ?? entry.id, index),
 title: entry.action,
 owner: entry.actorName ?? "System",
...
function backupRecords(entries: BackupRecord[]): ExecutiveRecord[] {
 return entries.slice(0, 6).map((entry, index) => ({
 id: toRecordId("backup", entry._id ?? entry.id, index),
 title: `${entry.type} backup`,
 owner: "System",
 status: entry.status,
 value: formatDateTime(entry.createdAt),
 }));
...
export function ExecutiveAccessPage({ moduleKey }: { moduleKey: ExecutiveModuleKey }) {
 ...
 const [liveDataLabel, setLiveDataLabel] = useState("Static module summary");
 ...
 setLiveDataLabel("Static module summary");

 if (!token) return;
 if (!["admin", "rbac", "audit-backup"].includes(moduleKey)) return;

 setIsLoadingLiveData(true);
 try {
 if (moduleKey === "admin" || moduleKey === "rbac") {
 const [rolesResult, permissionsResult, permissionAuditResult, access] = await Promise.all([
 requestBackend<Paginated<RoleRecord>>("/rbac/roles?limit=100", token),
 requestBackend<PermissionCatalogRecord[]>("/rbac/permissions", token),
 requestBackend<Paginated<PermissionAuditRecord>>("/rbac/audit-log?limit=50", token),
 fetchModuleAccess(),
 ]);
 const roles = rolesResult?.items ?? [];
 const permissions = permissionsResult ?? [];
 const permissionAudit = permissionAuditResult?.items ?? [];
 const permissionModules = new Set(permissions.map((permission) => permission.module));

 setEnabled(access?.adminPanelEnabled ?? true);
 setRecords(moduleKey === "rbac" ? roleRecords(roles) : [...roleRecords(roles).slice(0, 3), ...permissionAuditRecords(permissionAudit).slice(0, 3)]);
 setStats([
 { label: "Active roles", value: String(roles.length), icon: UsersRound },
 { label: "Permission modules", value: String(permissionModules.size), icon: ShieldCheck },
 { label: "Audit events", value: String(permissionAuditResult?.pagination?.total ?? permissionAudit.length), icon: FileText },
 { label: moduleKey === "admin" ? "Admin access" : "Full access roles", value: moduleKey === "admin" ? (access?.adminPanelEnabled === false ? "Disabled" : "Enabled") : String(roles.filter((role) => role.hasFullAccess).length), icon: BadgeCheck },
 ]);
 setLiveDataLabel("Live backend governance data");
 return;
 }

 const [auditResult, backups] = await Promise.all([
 requestBackend<Paginated<SystemAuditRecord>>("/audit/logs?limit=50", token),
 requestBackend<BackupRecord[]>("/backup/history", token),
 ]);
 const auditEntries = auditResult?.items ?? [];
 const backupEntries = backups ?? [];
 setRecords([...systemAuditRecords(auditEntries).slice(0, 3), ...backupRecords(backupEntries).slice(0, 3)]);
 setStats([
 { label: "Audit entries", value: String(auditResult?.pagination?.total ?? auditEntries.length), icon: FileText },
 { label: "Backups", value: String(backupEntries.length), icon: Download },
 { label: "Completed backups", value: String(backupEntries.filter((backup) => backup.status === "Completed").length), icon: BadgeCheck },
 { label: "Latest backup", value: backupEntries[0] ? new Date(backupEntries[0].createdAt).toLocaleDateString() : "None", icon: BarChart3 },
 ]);
 setLiveDataLabel("Live backend audit and backup data");
 } finally {
 setIsLoadingLiveData(false);
 }
 }, [config.records, config.stats, moduleKey, token]);
 ...
 downloadJson(`${moduleKey}-export.json`, { module: config.title, source: liveDataLabel, records, exportedAt: new Date().toISOString() });
 toast({ title: "Export downloaded", description: `${config.title} data exported.`, type: "success" });
 return;
 }
 if (action.toLowerCase().includes("review") || action.toLowerCase().includes("approve")) {
 toast({ title: "Review from source module", description: "This CEO page shows governance status. Use the connected admin/RBAC module for saved changes.", type: "info" });
 return;
 }
 toast({ title: action, description: "Action routed to the connected governance workflow where available.", type: "success" });
 };
 ...
 <p className="mt-2 inline-flex items-center gap-2 rounded-md border bg-background px-2.5 py-1 text-xs font-semibold text-muted-foreground">
 <BadgeCheck className="h-3.5 w-3.5 text-primary" />
 {isLoadingLiveData ? "Refreshing backend data..." : liveDataLabel}
 </p>
 </div>
 </div>
 </div>
 ...
 <p className="mt-2 text-sm text-muted-foreground">Search live records, refresh backend data, and export the current view.</p>
 </div>
 <div className="flex gap-2">
 <Button disabled={isLoadingLiveData} onClick={() => void loadLiveData()} type="button">
 <RefreshCw className={cn("h-4 w-4", isLoadingLiveData && "animate-spin")} />
 Refresh
 </Button>
 ...
 <Button onClick={() => runAction(`Review ${record.title}`)} size="sm" type="button" variant="outline">
 ...
 <button
 className="flex w-full items-center justify-between rounded-lg border bg-background px-4 py-3 text-left text-sm font-semibold transition-colors hover:border-primary/40 hover:bg-primary/5"
 key={action}
