...
import { formatDateTime as formatDateTimeShared } from "@shared/lib/utils-helpers";
...
function WorkflowBuilderPalette({ onAddStep }: { onAddStep: (type: WorkflowStep["type"]) => void }) {
 ...
 return (
 <button
 className="flex w-full items-center gap-3 rounded-xl border bg-background p-3 text-left transition-all hover:-translate-y-0.5 hover:border-primary/40"
 key={item.type}
...
function WorkflowCanvas({
 ...
 <button
 className="cursor-grab rounded-lg border bg-background p-2 transition-all hover:border-primary/40 active:cursor-grabbing"
 draggable
...
function WorkflowExecutionHistory({ history }: { history: Workflow["executionHistory"] }) {
 ...
 <div className="flex items-center gap-3 rounded-lg border bg-background p-3" key={log.stepId}>
...
function WorkflowFormModal({
 ...
 onClose: () => void;
 onSubmit: (input: WorkflowFormInput) => Promise<boolean>;
}) {
 ...
 return (
 <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground p-4 ">
 <motion.div
 ...
 {submitting ? "Saving..." : initialWorkflow ? "Save Workflow" : "Create Workflow"}
 </Button>
 </div>
 </motion.div>
 </div>
...
export function WorkflowsPage() {
 ...
 const canDeleteWorkflow = getStoredAuthSession()?.user.role !== "Manager";

 const loadWorkflows = useCallback(async ({ silent = false }: { silent?: boolean } = {}) => {
 ...
 ? "Your account is not authorized to view workflows."
 : result.message ?? "Workflows could not be loaded from the backend.",
 );
 }
 ...
 useEffect(() => {
 const refresh = () => void loadWorkflows({ silent: true });
 void loadWorkflows();
 ...
 const upsertWorkflow = async (input: WorkflowFormInput) => {
 try {
 if (editingWorkflow) {
 await apiUpdateWorkflow(editingWorkflow.id, input);
 toast({ title: "Workflow updated", description: `${input.name} was saved to the backend.`, type: "success" });
 } else {
 await apiCreateWorkflow(input);
 toast({ title: "Workflow created", description: `${input.name} was saved to the backend.`, type: "success" });
 }
 ...
 toast({ title: "Could not save workflow", description: (error as Error).message, type: "error" });
 return false;
 }
 };
 ...
 await loadWorkflows({ silent: true });
 toast({ title: "Workflow deleted", description: "The workflow was removed from the backend.", type: "warning" });
 } catch (error) {
 toast({ title: "Could not delete workflow", description: (error as Error).message, type: "error" });
 }
 }
 };
 ...
 try {
 await apiDuplicateWorkflow(workflow.id, `${workflow.name} Copy`);
 await loadWorkflows({ silent: true });
 toast({ title: "Workflow duplicated", description: "A backend copy was created as a draft.", type: "success" });
 } catch (error) {
 toast({ title: "Could not duplicate workflow", description: (error as Error).message, type: "error" });
 }
 };
 ...
 if (!workflow) return;
 try {
 const updated = await toggleWorkflowStatus(id);
 setWorkflows((current) => current.map((item) => item.id === id ? updated : item));
 toast({ title: `Workflow ${updated.status === "Active" ? "activated" : "paused"}`, description: `${workflow.name} is now ${updated.status}.`, type: "success" });
 } catch (error) {
 toast({ title: "Could not change workflow status", description: (error as Error).message, type: "error" });
 }
 };
 ...
 try {
 const execution = await apiExecuteWorkflow(id);
 await loadWorkflows({ silent: true });
 toast({
 title: execution.status === "completed" ? "Workflow executed" : "Workflow execution updated",
 description: `${workflow.name} is ${execution.status}.`,
 type: execution.status === "failed" ? "error" : "success",
 });
 } catch (error) {
 toast({ title: "Could not execute workflow", description: (error as Error).message, type: "error" });
 }
 };

 const openWorkflow = async (workflow: Workflow) => {
 const result = await fetchWorkflowExecutions(workflow.id);
 setEditingWorkflow(result.status === "ok" ? { ...workflow, executionHistory: result.data } : workflow);
 if (result.status === "error") {
 toast({ title: "Execution history unavailable", description: result.message ?? "Try again.", type: "warning" });
 }
 };
 ...
 {loadError && (
 <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">{loadError}</div>
 )}
 {loading && <p className="py-4 text-center text-sm text-muted-foreground">Loading workflows...</p>}
 ...
 <select className="h-11 rounded-md border bg-background px-3 text-sm" value={status} onChange={(e) => setStatus(e.target.value)}>
 <option>All</option>
 {workflowStatuses.map((item) => <option key={item}>{item}</option>)}
 </select>
 <select className="h-11 rounded-md border bg-background px-3 text-sm" value={sortBy} onChange={(e) => setSortBy(e.target.value as keyof Workflow)}>
 ...
 <Button size="sm" variant="outline" onClick={(event) => { event.stopPropagation(); void openWorkflow(workflow); }}><Eye className="h-4 w-4" />Open</Button>
 <Button size="sm" variant="outline" onClick={(event) => { event.stopPropagation(); void openWorkflow(workflow); }}><Edit3 className="h-4 w-4" />Edit</Button>
 <Button size="sm" variant="outline" onClick={(event) => { event.stopPropagation(); void duplicateWorkflow(workflow); }}><Copy className="h-4 w-4" /></Button>
 <Button size="sm" variant="outline" onClick={(event) => { event.stopPropagation(); void toggleStatus(workflow.id); }}>{workflow.status === "Active" ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}</Button>
 <Button size="sm" variant="outline" onClick={(event) => { event.stopPropagation(); void executeWorkflow(workflow.id); }}><Zap className="h-4 w-4" /></Button>
 {canDeleteWorkflow && <Button size="sm" variant="outline" onClick={(event) => { event.stopPropagation(); void deleteWorkflow(workflow.id); }}><Trash2 className="h-4 w-4" /></Button>}
 </div>
 </CardContent>
 </Card>
 );
