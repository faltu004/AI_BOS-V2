...
import { aiAssistantOpenEvent } from "@shared/platform/events";
...
export const FloatingAIAssistant = memo(function FloatingAIAssistant() {
 ...
 // Lets the dashboard sidebar's own "AI Analyzer" item open this same panel, since
 // the floating trigger button is hidden on the dashboard in favor of that entry.
 useEffect(() => {
 window.addEventListener(aiAssistantOpenEvent, openAssistant);
 return () => window.removeEventListener(aiAssistantOpenEvent, openAssistant);
 });
 ...
 "fixed inset-0 z-[95]",
 backdropBlurred
 ? "pointer-events-auto bg-slate-950/15 dark:bg-slate-950/25"
 : "pointer-events-none bg-transparent backdrop-blur-0",
 ].join(" ")
 : isDashboard
 ? "hidden"
 : "pointer-events-none fixed bottom-5 right-4 z-[70] sm:right-6"
 }
 ...
 className={[
 "pointer-events-auto fixed flex max-h-[calc(100dvh-1rem)] max-w-[calc(100vw-1rem)] flex-col overflow-hidden rounded-xl border border-primary/25 bg-card text-card-foreground shadow-2xl shadow-cyan-950/15 dark:border-cyan-200/25 dark:bg-slate-950 dark:text-slate-100 dark:shadow-cyan-950/30",
 ].join(" ")}
 onPointerDown={(event) => {
 event.stopPropagation();
 setBackdropBlurred(true);
 ...
 className="grid h-9 w-9 place-items-center rounded-lg text-muted-foreground transition hover:bg-muted hover:text-foreground dark:text-slate-300 dark:hover:bg-white dark:hover:text-white"
 type="button"
 onClick={() =>
 setExpanded((value) => {
 resetPanel(!value);
 ...
 : "rounded-bl-md border border-border bg-muted text-foreground dark:border-white/10 dark:bg-white/[0.08] dark:text-slate-100",
 ].join(" ")}
 >
 {message.content}
 </div>
 ...
 <div className="flex flex-col gap-1 rounded-2xl rounded-bl-md border border-border bg-muted px-3.5 py-2.5 text-sm text-muted-foreground dark:border-white/10 dark:bg-white/[0.08] dark:text-slate-300">
 <span className="flex items-center gap-2">
 <Loader2 className="animate-spin" size={15} aria-hidden="true" />
 Thinking with allowed data
 </span>
 <span className="text-xs opacity-75">Running on a local model — this can take up to a minute.</span>
 </div>
 </div>
 ) : null}
 </div>
 ...
 className="pointer-events-auto group inline-flex h-11 w-[min(22rem,calc(100vw-2rem))] items-center justify-between gap-3 rounded-full border border-white/20 bg-primary px-4 text-sm font-semibold text-primary-foreground shadow-xl shadow-primary/25 transition hover:-translate-y-0.5 hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background sm:w-[20rem]"
 type="button"
 onClick={openAssistant}
 >
 <span className="inline-flex min-w-0 items-center gap-2">
 <MessageSquareText className="shrink-0 transition group-hover:scale-105" size={18} aria-hidden="true" />
 <span className="truncate">AI Analyzer</span>
 </span>
 <span className="rounded-full bg-primary-foreground/18 px-2.5 py-1 text-xs font-semibold text-primary-foreground transition group-hover:bg-primary-foreground/24">
 Open
 </span>
 </button>
 ) : null}
