...
function sortRoomsByActivity(rooms: CollaborationRoom[]) {
...
export function CollaborationHub() {
 ...
 lastMessage: { body: message.body, authorId: message.authorId, createdAt: message.createdAt },
 ...
 setMessages((current) => (sameId(message.roomId, selectedRoomIdRef.current) ? mergeMessageList(current, message) : current));
 setRooms((current) =>
 sortRoomsByActivity(
 current.map((room) =>
 sameId(room._id, message.roomId)
 ...
 if (cancelled) return;
 setRooms(sortRoomsByActivity(roomList));
 ...
 fetchPinnedMessages(roomId, token),
 fetchNote(roomId, token),
 ])
 .then(([messageList, pinned, roomNote]) => {
 if (cancelled || !sameId(selectedRoomIdRef.current, roomId)) return;
 setMessages((current) =>
 mergeMessageLists(
 current.filter((message) => sameId(message.roomId, roomId)),
 messageList,
 ),
 );
 ...
 .catch((error: Error) => {
 if (!cancelled) {
 toast({ title: "Could not load room", description: error.message, type: "error" });
 }
 });

 markRoomRead(roomId, token).then(() => {
 setRooms((current) => current.map((room) => (sameId(room._id, roomId) ? { ...room, unreadCount: 0 } : room)));
 }).catch(() => undefined);
 ...
 // `room:join` is a client-initiated action tied to the socket instance that sent
 // it — it does NOT survive a disconnect/reconnect (a reconnect gets a brand-new
 // socket.id server-side, so the server's room membership from before is gone).
 // Without this, any reconnect (dev-server restarts, brief network drops, laptop
 // sleep/wake) silently stops this tab from receiving message/reaction/note/typing
 // events for the open room until the user manually switches rooms — which reads
 // as "messages don't load" since only messages have a polling fallback at all.
 useEffect(() => {
 ...
 if (!roomId) return;
 joinRoom(roomId).catch(() => undefined);
 fetchMessages(roomId, token).then((messageList) => {
 setMessages((current) => (sameId(roomId, selectedRoomIdRef.current) ? mergeMessageLists(current, messageList) : current));
 }).catch(() => undefined);
 }
 ...
 {searchResults && (
 <div className="mt-2 max-h-40 space-y-1 overflow-y-auto rounded-md border bg-background p-2 text-xs">
 ...
 className="min-h-40 flex-1 rounded-md border bg-background p-2 text-sm"
