          ...
          <>
            ...
            {integration.lastSyncSummary && (
              <p className="text-xs text-muted-foreground">
                Last sync: {integration.lastSyncSummary} {integration.lastSyncAt ? `(${new Date(integration.lastSyncAt).toLocaleString()})` : ""}
              </p>
            )}
            ...
            {logs && logs.length > 0 && (
              <div>
                ...
                {showLogs && (
                  <div className="mt-2 max-h-40 space-y-1 overflow-y-auto rounded-lg border bg-background/65 p-2">
                    {logs.map((log) => (
