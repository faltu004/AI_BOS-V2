...
import { ToastContext, type ToastInput, type ToastType } from "@/components/ui/toast-context";
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div className="pointer-events-none fixed right-4 top-4 z-[80] flex w-[calc(100vw-2rem)] max-w-sm flex-col gap-3 sm:right-6">
        <AnimatePresence>
          {items.map((item) => {
            ...
            return (
              <motion.div
                ...
                className="pointer-events-auto overflow-hidden rounded-2xl border bg-background/92 p-4 shadow-glass backdrop-blur-2xl"
              ...
              >
                <div className="flex items-start gap-3">
                  <span className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border", toastStyles[item.type])}>
