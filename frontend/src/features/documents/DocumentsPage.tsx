      ...
      >
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
          <Card className="bg-background/65">
            ...
            <CardContent>
              <div className="flex min-h-72 items-center justify-center rounded-lg border bg-card/70 p-6 text-center">
                <div>
            ...
            <Card className="bg-background/65">
              ...
              <CardContent className="space-y-3">
                {document.versions.map((version) => (
                  <div className="rounded-lg border bg-card/70 p-3" key={version.id}>
            ...
            <Card className="bg-background/65">
              ...
              <CardContent className="space-y-3">
                ...
                {document.sharedWith.map((share) => (
                  <div className="flex items-center justify-between gap-3 rounded-lg border bg-card/70 p-3 text-sm" key={share.user}>
      ...
      <CardContent className="space-y-4 p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {document.tags.map((tag) => (
            <span className="rounded-full border bg-background/60 px-2.5 py-1 text-xs font-semibold text-muted-foreground" key={tag}>
              {tag}
            </span>
          ))}
        </div>
...
export function DocumentsPage() {
  ...
  const uploadDocuments = (files: FileList | null) => {
    ...
    const uploaded = Array.from(files).map((file) => createDocumentFromFile(file, activeFolder, "Aman Verma"));
            ...
            <Card className="glass">
              ...
              <CardContent className="flex flex-wrap gap-2">
                {supportedFileTypes.map((type) => (
                  <span className="rounded-full border bg-background/65 px-3 py-1.5 text-xs font-semibold text-muted-foreground" key={type}>
                    {type}
                  </span>
                ))}
              </CardContent>
            ...
            <Card className="glass">
              <CardContent className="space-y-4 p-4">
                <div className="grid gap-3 lg:grid-cols-[1fr_160px_160px]">
                  ...
                  <select className="h-11 rounded-md border bg-background/75 px-3 text-sm" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}>
                    <option>All</option>
                    {supportedFileTypes.map((type) => (
                      <option key={type}>{type}</option>
                    ))}
                  </select>
                  <select className="h-11 rounded-md border bg-background/75 px-3 text-sm" value={tagFilter} onChange={(event) => setTagFilter(event.target.value)}>
                    <option>All</option>
                    {documentTags.map((tag) => (
                      <option key={tag}>{tag}</option>
                    ))}
