...
import { fetchEmployeeUsers, type BackendEmployee } from "@shared/employees/employees.api";
import {
  adminActivityLogs,
  adminAnalytics,
  adminAuditLogs,
...
import { canAccess, createEmptyRecord, downloadAdminBackup, generateTemporaryPassword } from "./admin.utils";
...
function toAdminUserRecord(user: BackendEmployee): AdminRecord {
  return {
    id: user.id,
    name: user.fullName,
    email: user.email,
...
export function AdminPage() {
  ...
  useEffect(() => {
    ...
    void fetchEmployeeUsers().then((result) => {
      if (!isMounted || result.status !== "ok" || result.data.length === 0) return;
      const userRecords = result.data.map(toAdminUserRecord);
      setModules((current) =>
        current.map((module) => (module.id === "users" ? { ...module, records: userRecords } : module)),
      );
    });
  ...
  const createUserWithTempPassword = () => {
    ...
    setModules((current) =>
      current.map((module) =>
        module.id === "users" ? { ...module, records: [newRecord, ...module.records] } : module,
      ),
    );
    setActivityLogs((current) => appendLog(current, "Created user with temporary password", "Users"));
    toast({ title: "User created with temporary password", description: `Temp password: ${tempPassword}`, type: "info" });
                ...
                <CardContent className="space-y-4 p-4">
                  <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
                     ...
                     {activeModule.id === "users" ? (
                       <Button onClick={createUserWithTempPassword} type="button">
